...
- **BaseService**：抽象类，提供静态方法 `get<T>`、`post<T>`、`put<T>`、`delete<T>`。各方法调用 `apiClient.*`，期望 `ApiResponse<T>`，并返回 `response.data.data`。子类设置 `protected static basePath`，通过 `getFullPath(path)` 拼 URL。
- **约定**：
